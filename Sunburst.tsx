...
const RINGS: Array<[number, number]> = [[37, 59], [59, 79], [79, 96]];
...
function arcPath(a0: number, a1: number, r0: number, r1: number): string {
  ...
  return `M${pt(a0, r1)}A${r1},${r1} 0 ${big} 1 ${pt(end, r1)}`
...
const Sector = memo(function Sector({ branch, hit, anyHover, q }: {
  branch: SunBranch; hit: string | null; anyHover: boolean; q: string;
}): React.JSX.Element {
  ...
  return (
    <g>
      {branch.arcs.map(a => {
        ...
        return (
          <g key={a.key}>
            <path className="sunarc" data-on={on ? 1 : 0}
              d={arcPath(a.a0, a.a1, r0, r1)} fill={h}
              opacity={dim ? 0.24 : (on || carry ? 1 : 1 - a.ring * 0.14)}
              onClick={() => drill(branch.name)}
              {...hoverBind({
                key: a.key, name: a.name, cost: a.cost, under: a.under, group: branch.group,
              })}>
...
function Core({ rootCost, label, kids }: {
  rootCost: number; label: string; kids: number;
}): React.JSX.Element {
  ...
  const inner = h
    ? <>
        ...
        <span className="s">{h.name}<br />
          <span className="dim">{pctOf(h.cost, rootCost).toFixed(pctOf(h.cost, rootCost) < 1 ? 2 : 1)}% of {label}</span>
        </span>
      </>
    : <>
        <span className="k">{label}</span>
        <span className="v">{amt(rootCost)}</span>
        <span className="s">{kids} line items<br />
...
const LegRow = memo(function LegRow({ branch, hue, on, dim }: {
  branch: SunBranch; hue: string; on: boolean; dim: boolean;
}): React.JSX.Element {
  ...
  const note = branch.folded
    ...
    : branch.items
      ? `${branch.items} item${branch.items === 1 ? "" : "s"}`
        + (kids.length ? " · " + kids.slice(0, 2).map(k => `${k.name} ${amt(k.cost)}`).join(" · ") : "")
      : "single line item · no further breakdown";

  return (
    <div className="legrow" data-on={on ? 1 : 0} data-dim={dim ? 1 : 0}
      {...hoverBind({
        key: branch.key, name: branch.name, cost: branch.cost, under: null, group: branch.group,
      })}>
      <span className="sw" style={{ background: hue }} />
      <button type="button" data-folded={branch.folded ? 1 : 0}
        onClick={() => drill(branch.name)}>{branch.name}</button>
    ...
    <div className="sun">
      ...
      <div className="sunlegend">
        {branches.map(b => {
          ...
          return (
            <LegRow key={b.name} branch={b} hue={pal.hue(b.group)} on={on}
              dim={(!!hk && !on) || (!!q && !b.key.toLowerCase().includes(q)
