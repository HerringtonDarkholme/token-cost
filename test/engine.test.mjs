...
for (let k = 0; k < 12; k++) {
  i++;
  req([{ type: "text", text: "reasoning ".repeat(40) },
       { type: "tool_use", id: "u" + k, name: "mcp__acme_corp__warehouse_query",
  ...
  req([{ type: "tool_use", id: "w" + k, name: "PatchFile",
         input: { path: `/x/mod${k}.${["zig", "nim", "zig", "ex"][k % 4]}`, contents: "CODE ".repeat(700) } }]);
...
const recon = ds => {
  ...
  if (Math.abs(t - ds.total) > Math.max(0.05, ds.total * 0.0005)) bad.push(`groups ${t} vs total ${ds.total}`);
  return bad;
};
...
if (dir) {
  ...
  const D1 = R.datasets["1h"], D5 = R.datasets["5m"];
  console.log(`  total ${money(D1.total)}  ·  density ${R.density.code.toFixed(2)}/${R.density.text.toFixed(2)} (${R.density.basis})`);
  ...
  ok(Object.keys(R.unpriced).length === 0 || true,
